...
{ 
   ...
   std::cout << "The following registers are tainted at the end of execution" << std::endl;
   list<REG>::iterator i;
   for(i = regsTainted.begin(); i != regsTainted.end(); i++){
      std::cout << *i << std::endl;
   }
   ...
   std::cout << "The following memory addresses are tainted at the end of execution" << std::endl;
   list<UINT64>::iterator from, to, cur, prev;
   cur = addressTainted.begin();
   prev = cur;
   from = prev;
   to = prev;
   cur++;
   while(cur != addressTainted.end()){
      ...
      if(*prev + 1 == *cur){
         ...
         std::cout << std::hex << "From " << *from << " to " << *to << std::endl;
         from = cur;
         to = cur;
         cur++;
         prev++;
